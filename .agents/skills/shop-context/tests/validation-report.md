...
## real-use-cca-shop Full Reference Mode

Validation note:

- The real-use CCA shop test files were created under `.agents/skills/shop-context/tests/real-use-cca-shop/`.
- Full Reference Mode was exercised on the real-use CCA shop router and machine-list CSV files.
- The complete draft `shop-reference.md` was generated.
- Compact Validation Mode was not used for this run.
- Scope stayed within ShopContext Layer 1.
- Required patches: none.
- Optional improvements: add more real-use machine list fields in future tests, such as work center, asset ID, and explicit operation support, to reduce mapping uncertainty.

## real-use-cca-shop User Setup Mode

Validation note:

- User Setup Mode was added as the default for real shop setup inputs.
- A user-facing CCA setup review example was added at `.agents/skills/shop-context/tests/real-use-cca-shop/user-setup-mode-example.md`.
- The example confirms that full `shop-reference.md` output is not generated by default during initial setup.
- The example asks 9 primary targeted questions and 1 optional evidence-retention question.
...
- Scope stayed within ShopContext Layer 1.
- Required patches: none.
- Optional improvements: add a future real-use example where the user answers setup questions and ShopContext updates the reference.
